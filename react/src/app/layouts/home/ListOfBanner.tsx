...
const theme = createTheme({
  ...
  const ActiveInactive = (props:any)=>{
    const {item,setloading,setOpenSnackbar,openSnackbar,_bannerList} = props;

    const _updateBannerStatus =()=>{
      setloading(true)
      ...
      return(
        <Box style={{display:'flex',alignItems:'center',justifyContent:'center'}}>
          <IOSSwitch size='small' sx={{ m: 1 }} defaultChecked={item.active_inactive} onChange={_updateBannerStatus} />
        </Box>
      )
  }
  ...
  const ActionView = (props:any)=>{
    ...
    return(
        <Box style={{display:'flex',alignItems:'center',justifyContent:'center'}}>
          <Button onClick={()=>navigation('/admin/banner/add-banner',{ state: {TAG:'EDIT',data:item} })} variant="outlined" color={'primary'} size="small" startIcon={<GoPencil color={_.colors.colorSubTitle} style={{marginRight:-8,marginTop:2,marginBottom:2}} size={16}/>}/>
          <Button onClick={()=>{
          setdialog(true)
          setidForDelete(item)
  ...
  const RestaurantLogo = (props:any)=>{
    ...
    return(
        <Box sx={{}}>
          <img src={FILE_URL+FILE_PATH.BANNER_IMAGE+item.image} 
          style={{height:'100px',width:'180px',borderRadius:10,objectFit:'cover'}}/>
        </Box>
    )
  }
...
export default function ListOfBanner() {
  ...
  const [totalNumberOfBanners, settotalNumberOfBanners] = useState(0)
...
const pageLength = 10
  const _bannerList = ()=>{
    ...
    var config = {
      method: 'post',
      url: BASE_URL+PATH.LIST_OF_BANNER,
      headers: { 
        'Content-Type': 'application/x-www-form-urlencoded'
      },
      data : data
    ...
    .then(function (response:any) {
      console.log(JSON.stringify(response.data));
      if(response.data.status){
        settotalNumberOfBanners(response.data.data.length)
        const paginate = (array:any,page_size:any,page_number:any) =>{
          return array.slice((page_number - 1) * page_size, page_number * page_size);
        }
        setbannerList(paginate(response.data.data.reverse(), pageLength, currentPage))
      }else{
  ...
  const _deleteBanner = ()=>{
    ...
    var config = {
      method: 'post',
      url: BASE_URL+PATH.DELETE_BANNER,
      headers: { 
        'Content-Type': 'application/x-www-form-urlencoded'
      },
      data : data
    ...
    .then(function (response:any) {
      ...
      setidForDelete(null)
      if(response.data.status){
        setOpenSnackbar({ ...openSnackbar, open: true,message:'Delete banner successful !',type:'success' });
        _bannerList()
      }else{
        setOpenSnackbar({ ...openSnackbar, open: true,message:'Something went wrong !',type:'error' });
      }
  ...
  return (
    ...
    <Grid style={{display:'flex',flexDirection:'row',alignItems:'center'}}>
        <Typography variant="h1" style={{fontSize:20,color:_.colors.colorBlack,fontWeight:'bold'}} component="div">Banner List</Typography>
        <Typography variant="h1" style={{fontSize:20,color:_.colors.colorBlack,fontWeight:'bold',backgroundColor:_.colors.colorOrangeDisable,padding:10,borderRadius:10,marginLeft:10}} component="div">{totalNumberOfBanners}</Typography>
    </Grid>
    <Grid lg={12} sm={12} md={12} xs={12} style={{padding:10,marginBottom:'10%'}}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'flex-end',marginBottom:2 }}>
            ...
            <Box sx={{display:'flex',width:'70%',alignItems:'center',justifyContent:'flex-end'}}>
                <Button onClick={()=>navigation('/admin/banner/add-banner')} sx={{backgroundColor:'#FF9A03',":hover":{backgroundColor:'#E18701'},marginBottom:0.5,marginLeft:2}} size='small' variant="contained">Add Banner</Button>
            </Box>
        </Box>
        ...
        :<Box sx={{overflowX: 'auto', width: '100%'}}><Table sx={{minWidth:'900px'}} aria-label="simple table">
          <TableHead>
            <TableRow style={{backgroundColor:'#F0F0F0'}}>
              ...
              <TableCell align="center" style={{color:_.colors.colorBlack}}>IMAGE</TableCell>
          ...
          <TableBody>
            {bannerList.map((item:any,index:any) => (
              <TableRow
                ...
                <TableCell align="center"><Typography variant="h2" sx={{marginLeft:1,maxWidth:'200px',textAlign:'center',fontSize:14,color:_.colors.colorTitle,display: '-webkit-box',overflow: 'hidden',WebkitBoxOrient: 'vertical',WebkitLineClamp:2}} component="div">{item?.restaurant?.restaurant_name?item?.restaurant?.restaurant_name:'N/A'}</Typography></TableCell>
                <TableCell align="center" >{<ActiveInactive item={item} setloading={setloading} setOpenSnackbar={setOpenSnackbar} openSnackbar={openSnackbar} _bannerList={_bannerList}/>}</TableCell>
                <TableCell align="center">{<ActionView item={item} setdialog={setdialog} setidForDelete={setidForDelete}/>}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table></Box>}
        {totalNumberOfBanners !== 0 && <Pagination page={currentPage} onChange={(event: React.ChangeEvent<unknown>, page: number)=>{setcurrentPage(page)}} count={Math.ceil(totalNumberOfBanners/pageLength)} sx={{marginTop:3}} />}
        </TableContainer>
      </Grid>
    </Grid>
