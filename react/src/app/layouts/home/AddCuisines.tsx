...
function AddCuisines() {
    ...
    const _addCuisines = ()=>{
        if(name.length === 0 ){
            ...
            var config = {
            method: 'post',
            url: BASE_URL+PATH.ADD_CUISINES,
            headers: { 
            ...
            .then(function (response:any) {
                ...
                if(response.data.status){
                    ...
                    navigation('/admin/cuisines/list-of-cuisines') 
                }else{
    ...
    const _cuisinesUpdate = ()=>{
        if(name.length === 0){
            ...
            .then(function (response:any) {
                ...
                if(response.data.status){
                    setname('')
                    setOpenSnackbar({ ...openSnackbar, open: true,message:'update addons successful !',type:'success' });
                    navigation('/admin/cuisines/list-of-cuisines') 
                }else{
    ...
    const _clickSave=()=>{
        state?_cuisinesUpdate():_addCuisines()
    }
        ...
        <Box sx={{display:'flex', p: 2}}>
            <Grid container lg={12} sm={12} md={12} xs={12}>
                <Grid lg={12} sm={12} md={12} xs={12} sx={{display:'flex',flexDirection:'row',alignItems:'center'}}>
                    ...
                    <Link underline='none' onClick={()=>navigation('/admin/orders/all')} sx={{color:_.colors.colorDarkGray,":hover":{color:_.colors.colorOrange}}}>cuisines</Link>
                    <CgFormatSlash color={_.colors.colorDarkGray} size={20}/>
                    <Typography variant="h1" style={{fontSize:15,color:_.colors.colorTitle}} component="div">{state?'edit-cuisines':'add-cuisines'}</Typography>
                </Grid>
                <Box sx={{width:'100%',display:'flex',flexDirection:'row',alignItems:'center',marginTop:1.5}}>
                    <Typography variant="h6" style={{fontSize:20,color:_.colors.colorTitle,}} component="div">{state?'Edit Cuisines':'Add Cuisines'}</Typography>
                </Box>
                ...
                <Grid lg={12} sm={12} md={12} xs={12} sx={{}}>
                    ...
                    <Box sx={{display:'flex',flexDirection:'row',alignItems:'center',}}>
                        <Box sx={{display:'flex',flexDirection:'row',alignItems:'center',position:'absolute'}}>
                            <Typography variant="h6" sx={{fontSize:16,color:_.colors.colorTitle,marginRight:0.6}} component="div">Cuisines name</Typography>
                            <FaStarOfLife color={_.colors.colorRed} size={8}/>
                        </Box>
                        <Box sx={{width:'40%',display:'flex',flexDirection:'row',marginLeft:28}}>
                            <TextField value={name} onChange={(prop)=>setname(prop.target.value)} color='secondary' size='small' label="Enter Cuisines name" variant="outlined"
                            sx={{width:'100%'}} />
                        </Box>
                    </Box>
                </Grid>
                <Box sx={{width:'100%',display:'flex',flexDirection:'row',alignItems:'center',marginTop:10,marginBottom:10}}>
                    ...
                    <Button onClick={()=>navigation('/admin/cuisines/list-of-cuisines')} sx={{width:'15%',backgroundColor:_.colors.colorGray2,":hover":{backgroundColor:'#A0A0A0'},marginLeft:2}} size='medium' variant="contained">Cancel</Button>
                </Box>
            </Grid>
        </Box>
