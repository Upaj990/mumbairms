...
function AddManager() {
    ...
    const _updateMangerDetils =()=>{
        ...
        .then(function (response:any) {
            ...
            console.log(JSON.stringify(response.data));
            if(response.data.status){
                setOpenSnackbar({ ...openSnackbar, open: true,message:'update manager successful !',type:'success' });
                navigation('/admin/manager/list-of-manager') 
            }else{
    ...
    const _updateManager =()=>{
        if(name.trim().length === 0 || phone.trim().length === 0 || email.trim().length === 0){
            ...
            setOpenSnackbar({ ...openSnackbar, open: true,message:'Manager profile is required.',type:'error' });
    ...
    const _addIdentityProof = (item:any)=>{
        ...
        .then(function (response:any) {
            ...
            if(response.data.status){
                ...
                setpassword('')
                setconfirmPassword('')
                setOpenSnackbar({ ...openSnackbar, open: true,message:'Add manager successful !',type:'success' });
                navigation('/admin/manager/list-of-manager')
            }else{
                setOpenSnackbar({ ...openSnackbar, open: true,message:'Something went wrong !',type:'error' });
            }
    ...
    const _addManagers =()=>{
        if(name.length === 0 || phone.length === 0 || email.length === 0 || password.length === 0 || confirmPassword.length === 0){
            ...
            data.append('admin_type', 'MANAGER');
    ...
    const _onClickSubmit = ()=>{
        state?_updateManager():_addManagers()
    }
        ...
        <Box sx={{display:'flex', p: 2}}>
            <Grid container lg={12} sm={12} md={12} xs={12}>
                <Grid lg={12} sm={12} md={12} xs={12} sx={{display:'flex',flexDirection:'row',alignItems:'center'}}>
                    ...
                    <Link underline='none' onClick={()=>navigation('/admin/orders/all')} sx={{color:_.colors.colorDarkGray,":hover":{color:_.colors.colorOrange}}}>manager</Link>
                    <CgFormatSlash color={_.colors.colorDarkGray} size={20}/>
                    <Typography variant="h1" style={{fontSize:15,color:_.colors.colorTitle}} component="div">{state?'Edit-manager':'Add-manager'}</Typography>
                </Grid>
                <Box sx={{width:'100%',display:'flex',flexDirection:'row',alignItems:'center',marginTop:1.5}}>
                    <Typography variant="h6" style={{fontSize:20,color:_.colors.colorTitle,}} component="div">{state?'Edit Manager':'Add Manager'}</Typography>
                </Box>
                ...
                <Grid lg={12} sm={12} md={12} xs={12} sx={{}}>
                    ...
                    <Box sx={{display:'flex',flexDirection:'row',alignItems:'center',marginTop:3,}}>
                        ...
                        <Box sx={{width:'40%',display:'flex',flexDirection:'row',marginLeft:28}}>
                            ...
                            color='secondary' size='small' label="Enter phone number" variant="outlined"
                            sx={{width:'100%'}} />
                        </Box>
                    </Box>
                    {/* Select Restaurant */}
                ...
                <Box sx={{width:'100%',display:'flex',flexDirection:'row',alignItems:'center',marginTop:10,marginBottom:10}}>
                    ...
                    <Button onClick={()=>navigation('/admin/manager/list-of-manager')} sx={{width:'15%',backgroundColor:_.colors.colorGray2,":hover":{backgroundColor:'#A0A0A0'},marginLeft:2}} size='medium' variant="contained">Cancel</Button>
                </Box>
            </Grid>
        </Box>
