...
const theme = createTheme({
  ...
  const ActiveInactive = (props:any)=>{
    const {item,setloading,setOpenSnackbar,openSnackbar,_listOfAddOns} = props;

    const _updateAddonsStatus =()=>{
      setloading(true)
      ...
      var config = {
      method: 'post',
      url: BASE_URL+PATH.UPDATE_ADDONS,
      headers: { 
      ...
      return(
        <Box style={{display:'flex',alignItems:'center',justifyContent:'center'}}>
            <IOSSwitch size='small' sx={{ m: 1 }} defaultChecked={item.active_inactive} onChange={_updateAddonsStatus}/>
        </Box>
      )
  }
  ...
  const ActionView = (props:any)=>{
    ...
    return(
        <Box style={{display:'flex',alignItems:'center',justifyContent:'center'}}>
          <Button onClick={()=>navigation('/admin/add-ons/add-add-ons',{ state: {TAG:'EDIT',data:item} })} variant="outlined" color={'primary'} size="small" startIcon={<GoPencil color={_.colors.colorSubTitle} style={{marginRight:-8,marginTop:2,marginBottom:2}} size={16}/>}/>
          <Button onClick={()=>{
            setdialog(true)
            setidForDelete(item)
...
export default function ListOfAddOns() {
  ...
  const [totalNumberOfAddons, settotalNumberOfAddons] = useState(0)
  ...
  const _listOfAddOns = ()=>{
    ...
    .then(function (response:any) {
      console.log(JSON.stringify(response.data));
      if(response.data.status){
        settotalNumberOfAddons(response.data.data.length)
        const paginate = (array:any,page_size:any,page_number:any) =>{
          return array.slice((page_number - 1) * page_size, page_number * page_size);
        }
        setaddOnsList(paginate(response.data.data.reverse(), pageLength, currentPage))
      }
    })
    .catch(function (error:any) {
      console.log(error);
  ...
  const _deleteAddons = ()=>{
    ...
    var config = {
      method: 'post',
      url: BASE_URL+PATH.DELETE_ADDONS,
      headers: { 
        'Content-Type': 'application/x-www-form-urlencoded'
      },
      data : data
    ...
    .then(function (response:any) {
      ...
      setidForDelete(null)
      if(response.data.status){
        setOpenSnackbar({ ...openSnackbar, open: true,message:'Delete addons successful !',type:'success' });
        _listOfAddOns()
      }else{
        setOpenSnackbar({ ...openSnackbar, open: true,message:'Something went wrong !',type:'error' });
      }
  ...
  return (
    ...
    <Grid style={{display:'flex',flexDirection:'row',alignItems:'center'}}>
        <Typography variant="h1" style={{fontSize:20,color:_.colors.colorBlack,fontWeight:'bold'}} component="div">Add-ons List</Typography>
        <Typography variant="h1" style={{fontSize:20,color:_.colors.colorBlack,fontWeight:'bold',backgroundColor:_.colors.colorOrangeDisable,padding:10,borderRadius:10,marginLeft:10}} component="div">{totalNumberOfAddons}</Typography>
    </Grid>
    <Grid lg={12} sm={12} md={12} xs={12} style={{padding:10,marginBottom:'10%'}}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'flex-end',marginBottom:2 }}>
            ...
            <Box sx={{display:'flex',width:'70%',alignItems:'center',justifyContent:'flex-end'}}>
                <Button onClick={()=>navigation('/admin/add-ons/add-add-ons')} sx={{backgroundColor:'#FF9A03',":hover":{backgroundColor:'#E18701'},marginBottom:0.5,marginLeft:2}} size='small' variant="contained">Add Add-ons</Button>
            </Box>
        </Box>
          ...
          <TableBody>
            {addOnsList.map((item:any,index:any) => (
              <TableRow
                ...
                <TableCell align="center" >{<ActiveInactive item={item} setloading={setloading} setOpenSnackbar={setOpenSnackbar} openSnackbar={openSnackbar} _listOfAddOns={_listOfAddOns}/>}</TableCell>
                <TableCell align="center">{<ActionView item={item} setdialog={setdialog} setidForDelete={setidForDelete}/>}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table></Box>}
        {totalNumberOfAddons !== 0 && <Pagination page={currentPage} onChange={(event: React.ChangeEvent<unknown>, page: number)=>{setcurrentPage(page)}} count={Math.ceil(totalNumberOfAddons/pageLength)} sx={{marginTop:3}} />}
        </TableContainer>
      </Grid>
    </Grid>
