...
const theme = createTheme({
  ...
  const ActiveInactive = (props:any)=>{
    const {item,setloading,setOpenSnackbar,openSnackbar,_listOfManagers} = props;

    const _updateMangerStatus =()=>{
      setloading(true)
      ...
      return(
        <Box style={{display:'flex',alignItems:'center',justifyContent:'center'}}>
            <IOSSwitch sx={{ m: 1 }} defaultChecked={item.active_inactive} onChange={_updateMangerStatus}  />
        </Box>
      )
  }
  ...
  const ActionView = (props:any)=>{
    ...
    return(
        <Box style={{display:'flex',alignItems:'center',justifyContent:'center'}}>
          <Button onClick={()=>navigation('/admin/manager/add-manager',{ state: {TAG:'EDIT',data:item} })} variant="outlined" color={'primary'} size="small" startIcon={<GoPencil color={_.colors.colorSubTitle} style={{marginRight:-8,marginTop:2,marginBottom:2}} size={16}/>}/>
          <Button onClick={()=>{
            setdialog(true)
            setidForDelete(item)
...
export default function ListOfManager() {
  ...
  const [totalNumberOfManagers, settotalNumberOfManagers] = useState(0)
  ...
  const _listOfManagers = ()=>{
    ...
    .then(function (response:any) {
      ...
      console.log(JSON.stringify(response.data));
      if(response.data.status){
        settotalNumberOfManagers(response.data.data.length)
        const paginate = (array:any,page_size:any,page_number:any) =>{
          return array.slice((page_number - 1) * page_size, page_number * page_size);
        }
        setmanagersList(paginate(response.data.data.reverse(), pageLength, currentPage))
      }else{
  ...
  const _deleteManager = ()=>{
    ...
    .then(function (response:any) {
      ...
      setidForDelete(null)
      if(response.data.status){
        setOpenSnackbar({ ...openSnackbar, open: true,message:'Delete manager successful !',type:'success' });
        _listOfManagers()
      }else{
        setOpenSnackbar({ ...openSnackbar, open: true,message:'Something went wrong !',type:'error' });
      }
  ...
  return (
    ...
    <Grid style={{display:'flex',flexDirection:'row',alignItems:'center'}}>
      <Typography variant="h1" style={{fontSize:20,color:_.colors.colorBlack,fontWeight:'bold'}} component="div">Manager List</Typography>
      <Typography variant="h1" style={{fontSize:20,color:_.colors.colorBlack,fontWeight:'bold',backgroundColor:_.colors.colorOrangeDisable,padding:10,borderRadius:10,marginLeft:10}} component="div">{totalNumberOfManagers}</Typography>
    </Grid>
    <Grid lg={12} sm={12} md={12} xs={12} style={{padding:10,marginBottom:'10%'}}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'flex-end',marginBottom:2 }}>
            ...
            <Box sx={{display:'flex',width:'70%',alignItems:'center',justifyContent:'flex-end'}}>
                <Button onClick={()=>navigation('/admin/manager/add-manager')} sx={{backgroundColor:'#FF9A03',":hover":{backgroundColor:'#E18701'},marginBottom:0.5,marginLeft:2}} size='small' variant="contained">{mediaQuery ? 'Add New manager' : 'New'}</Button>
            </Box>
        </Box>
        ...
        :<Box sx={{overflowX: 'auto', width: '100%'}}><Table sx={{minWidth: 650}} aria-label="simple table">
          ...
          <TableBody>
            {managersList.map((row:any,index:any) => (
              <TableRow
                ...
                <TableCell align="center" >{<ActiveInactive item={row} setloading={setloading} setOpenSnackbar={setOpenSnackbar} openSnackbar={openSnackbar} _listOfManagers={_listOfManagers}/>}</TableCell>
                <TableCell align="center">{<ActionView item={row} setdialog={setdialog} setidForDelete={setidForDelete}/>}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table></Box>}
        {totalNumberOfManagers !== 0 && <Pagination page={currentPage} onChange={(event: React.ChangeEvent<unknown>, page: number)=>{setcurrentPage(page)}} count={Math.ceil(totalNumberOfManagers/pageLength)} sx={{marginTop:3}} />}
        </TableContainer>
      </Grid>
    </Grid>
