...
function AddWaiter() {
    ...
    const _editData =()=>{
        ...
        userDetails?.user.admin_type === 'MANAGER' || userDetails?.user.admin_type === 'WAITER' || userDetails?.user.admin_type === 'CHEF'?setrestaurant(userDetails?.user.restaurant):setrestaurant(state?.data?.restaurant)
    ...
    const _updateMangerDetils =()=>{
        ...
        .then(function (response:any) {
            ...
            console.log(JSON.stringify(response.data));
            if(response.data.status){
                setOpenSnackbar({ ...openSnackbar, open: true,message:'update waiter successful !',type:'success' }); 
                navigation('/admin/waiter/list-of-waiter')
            }else{
    ...
    const _updateWaiter =()=>{
        if(name.length === 0 || phone.length === 0 || email.length === 0){
            ...
            setOpenSnackbar({ ...openSnackbar, open: true,message:'Waiter profile is required.',type:'error' });
    ...
    const _addIdentityProof = (item:any)=>{
        ...
        .then(function (response:any) {
            ...
            if(response.data.status){
                ...
                setpassword('')
                setconfirmPassword('')
                setOpenSnackbar({ ...openSnackbar, open: true,message:'Add waiter successful !',type:'success' });
                navigation('/admin/waiter/list-of-waiter')
            }else{
                setOpenSnackbar({ ...openSnackbar, open: true,message:'Something went wrong !',type:'error' });
            }
    ...
    const _onClickSubmit = ()=>{
        state?_updateWaiter():_addWaiter()
    }
        ...
        <Box sx={{display:'flex', p: 2}}>
            <Grid container lg={12} sm={12} md={12} xs={12}>
                <Grid lg={12} sm={12} md={12} xs={12} sx={{display:'flex',flexDirection:'row',alignItems:'center'}}>
                    ...
                    <Link underline='none' onClick={()=>navigation('/admin/orders/all')} sx={{color:_.colors.colorDarkGray,":hover":{color:_.colors.colorOrange}}}>waiter</Link>
                    <CgFormatSlash color={_.colors.colorDarkGray} size={20}/>
                    <Typography variant="h1" style={{fontSize:15,color:_.colors.colorTitle}} component="div">{state?'Edit-waiter':'Add-waiter'}</Typography>
                </Grid>
                <Box sx={{width:'100%',display:'flex',flexDirection:'row',alignItems:'center',marginTop:1.5}}>
                    <Typography variant="h6" style={{fontSize:20,color:_.colors.colorTitle,}} component="div">{state?'Edit Waiter':'Add Waiter'}</Typography>
                </Box>
                ...
                <Grid lg={12} sm={12} md={12} xs={12} sx={{}}>
                    ...
                    <Box sx={{display:'flex',flexDirection:'row',alignItems:'center',marginTop:3,}}>
                        ...
                        <Box sx={{width:'40%',display:'flex',flexDirection:'row',marginLeft:28}}>
                            <TextField value={email} onChange={(prop)=>setemail(prop.target.value)} id="outlined-basic" color='secondary' size='small' label="Enter email addres" variant="outlined"
                            sx={{width:'100%'}} />
                        </Box>
                    </Box>
                    {/* Select user */}
                        ...
                        <Box sx={{width:'100%',display:'flex',flexDirection:'row'}}>
                            <Box sx={{width:'40%',display:'flex',flexDirection:'column',marginLeft:28}}>
                                ...
                                <Box sx={{height:'200px',width:{xs: '140px', sm: '340px'},marginTop:2,position:'relative'}}>
                                    ...
                                    style={{height:'200px',width:mediaQuery?'360px':'140px',borderRadius:10,objectFit:'cover',marginBottom:10}}/>
                                </Box>
                            </Box>
                        </Box>
                    </Box>
                ...
                <Box sx={{width:'100%',display:'flex',flexDirection:'row',alignItems:'center',marginTop:10,marginBottom:10}}>
                    ...
                    <Button onClick={()=>navigation('/admin/waiter/list-of-waiter')} sx={{width:'15%',backgroundColor:_.colors.colorGray2,":hover":{backgroundColor:'#A0A0A0'},marginLeft:2}} size='medium' variant="contained">Cancel</Button>
                </Box>
            </Grid>
        </Box>
