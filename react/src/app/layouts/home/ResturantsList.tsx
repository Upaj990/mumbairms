...
const theme = createTheme({
  ...
  const ActiveInactive = (props:any)=>{
    const {item,setloading,setOpenSnackbar,openSnackbar,_list_of_resturants} = props;

    const _updateRestaurantStatus =()=>{
      setloading(true)
    ...
    return(
      <Box style={{display:'flex',alignItems:'center',justifyContent:'center'}}>
        <IOSSwitch sx={{ m: 1 }} defaultChecked={item.active_inactive} onChange={_updateRestaurantStatus} />
      </Box>
    )
  }
  ...
  const ActionView = (props:any)=>{
    ...
    return(
      <Box style={{display:'flex',alignItems:'center',justifyContent:'center'}}>
        ...
        onClick={
          ()=>{
            localStorage.setItem('pick','pickedused');
            localStorage.setItem('pickdata',JSON.stringify(item));
            navigation('/admin/restaurant/add-new-restaurant',
        { state: {TAG:'EDIT',data:item} })
        ...
        startIcon={<GoPencil color={_.colors.colorSubTitle} 
        style={{marginRight:-8,marginTop:2,marginBottom:2}} size={16}/>}/>

        
        {userDetails?.user.admin_type === 'SUPER'&&
        <Button onClick={()=>{
          setdialog(true)
          setidForDelete(item)
  ...
  const RestaurantLogo = ({item}:{item:any})=>{
    return(
        <Box sx={{}}>
          <img src={FILE_URL+item?.restaurant_logo?.path} 
...
export default function ResturantsList() {
  ...
  const [totalNumberOfResturants, settotalNumberOfResturants] = useState(0)
    ...
    .then(function (response:any) {
      ...
      if(response.data.status){
        if(userDetails?.user.admin_type === 'MANAGER' || userDetails?.user.admin_type === 'WAITER' || userDetails?.user.admin_type === 'CHEF'){
          settotalNumberOfResturants(response.data.data.filter((a:any)=>a._id === userDetails?.user.restaurant._id).length)
          const paginate = (array:any,page_size:any,page_number:any) =>{
            return array.slice((page_number - 1) * page_size, page_number * page_size);
          }
          setresturantList(paginate(response.data.data.filter((a:any)=>a._id === userDetails?.user.restaurant._id).reverse(), pageLength, currentPage))
        }else{
          settotalNumberOfResturants(response.data.data.length)
          const paginate = (array:any,page_size:any,page_number:any) =>{
            return array.slice((page_number - 1) * page_size, page_number * page_size);
          }
          setresturantList(paginate(response.data.data.reverse(), pageLength, currentPage))
        }
      }
    })
    .catch(function (error:any) {
  ...
  const _deleteResturants = ()=>{
    ...
    var config = {
      method: 'post',
      url: BASE_URL+PATH.DELETE_RESTAURANT,
      headers: { 
        'Content-Type': 'application/x-www-form-urlencoded'
      },
      data : data
    ...
    .then(function (response:any) {
      ...
      setidForDelete(null)
      if(response.data.status){
        setOpenSnackbar({ ...openSnackbar, open: true,message:'Delete resturant successful !',type:'success' });
        _list_of_resturants()
      }else{
        setOpenSnackbar({ ...openSnackbar, open: true,message:'Something went wrong !',type:'error' });
      }
  ...
  function Order_Id() {
    ...
    axios(config)
      .then(function (response: any) {
        ...
        console.log(JSON.stringify(response.data));
        if (response.data.status) {
          if (_order_id != response.data.data.order_id) {
            _order_id = response.data.data.order_id;
  ...
  return (
    ...
    <Grid style={{display:'flex',flexDirection:'row',alignItems:'center'}}>
      <Typography variant="h1" style={{fontSize:20,color:_.colors.colorBlack,fontWeight:'bold'}} component="div">Restaurant List</Typography>
      <Typography variant="h1" style={{fontSize:20,color:_.colors.colorBlack,fontWeight:'bold',backgroundColor:_.colors.colorOrangeDisable,padding:10,borderRadius:10,marginLeft:10}} component="div">{totalNumberOfResturants}</Typography>
    </Grid>
    <Grid lg={12} sm={12} md={12} xs={12} style={{padding:10,marginBottom:'10%'}}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'flex-end',marginBottom:2 }}>
            ...
            <Box sx={{display:'flex',width:'70%',alignItems:'center',justifyContent:'flex-end'}}>
                <Button onClick={()=>navigation('/admin/restaurant/add-new-restaurant')} sx={{backgroundColor:'#FF9A03',":hover":{backgroundColor:'#E18701'},marginBottom:0.5,marginLeft:2}} size='small' variant="contained">{mediaQuery ? 'Add New Resturant' : 'New'}</Button>
            </Box>}
        </Box>

        {totalNumberOfResturants === 0?
          <Box sx={{width:'900px'}}>
            <Empty/>
          </Box>
        :<Box sx={{width: '100%', overflowX: 'auto'}}><Table sx={{minWidth:'900px'}} aria-label="simple table">
          <TableHead>
            <TableRow style={{backgroundColor:'#F0F0F0'}}>
              ...
              <TableCell align="center" style={{color:_.colors.colorBlack}}>OWNER</TableCell>
              <TableCell align="center" style={{color:_.colors.colorBlack}}>PHONE</TableCell>
          ...
          <TableBody>
            {resturantList.map((item:any,index:any) => (
              <TableRow
                key={item._id}
                ...
                <TableCell align="center"><Typography variant="h2" sx={{marginLeft:1,maxWidth:'200px',textAlign:'start',fontSize:14,color:_.colors.colorTitle,display: '-webkit-box',overflow: 'hidden',WebkitBoxOrient: 'vertical',WebkitLineClamp:2}} component="div">{item.restaurant_name}</Typography></TableCell>
                <TableCell align='center'>{item.restaurant_email}</TableCell>
                <TableCell align='center'>{item.restaurant_contact_number}</TableCell>
                ...
                _list_of_resturants={_list_of_resturants}
                />}
                </TableCell>
                <TableCell align="center">{
        ...
        {totalNumberOfResturants !== 0 && <Pagination page={currentPage} onChange={(event: React.ChangeEvent<unknown>, page: number)=>{setcurrentPage(page)}} count={Math.ceil(totalNumberOfResturants/pageLength)} sx={{marginTop:3}} />}
        </TableContainer>
      </Grid>
    </Grid>
