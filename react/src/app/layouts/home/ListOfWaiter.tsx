...
const theme = createTheme({
  ...
  const ActiveInactive = (props:any)=>{
    const {item,setloading,setOpenSnackbar,openSnackbar,_listOfWaiters} = props;

    const _updateWaiterStatus =()=>{
      setloading(true)
      ...
      return(
        <Box style={{display:'flex',alignItems:'center',justifyContent:'center'}}>
            <IOSSwitch sx={{ m: 1 }} defaultChecked={item.active_inactive} onChange={_updateWaiterStatus}  />
        </Box>
      )
  }
  ...
  const ActionView = (props:any)=>{
    ...
    return(
        <Box style={{display:'flex',alignItems:'center',justifyContent:'center'}}>
            <Button onClick={()=>navigation('/admin/waiter/add-waiter',{ state: {TAG:'EDIT',data:item} })} variant="outlined" color={'primary'} size="small" startIcon={<GoPencil color={_.colors.colorSubTitle} style={{marginRight:-8,marginTop:2,marginBottom:2}} size={16}/>}/>
            <Button onClick={()=>{
              setdialog(true)
              setidForDelete(item)
...
export default function ListOfWaiter() {
  const navigation = useNavigate()
  ...
  const [totalNumberOfWaiters, settotalNumberOfWaiters] = useState(0)
    ...
    .then(function (response:any) {
      ...
      if(response.data.status){
        if(userDetails?.user.admin_type === 'MANAGER' || userDetails?.user.admin_type === 'WAITER' || userDetails?.user.admin_type === 'CHEF'){
          settotalNumberOfWaiters(response.data.data.filter((a:any)=>a.restaurant._id === userDetails?.user.restaurant._id).length)
          const paginate = (array:any,page_size:any,page_number:any) =>{
            return array.slice((page_number - 1) * page_size, page_number * page_size);
          }
          setwaitersList(paginate(response.data.data.filter((a:any)=>a.restaurant._id === userDetails?.user.restaurant._id).reverse(), pageLength, currentPage))
        }else{
          settotalNumberOfWaiters(response.data.data.length)
          const paginate = (array:any,page_size:any,page_number:any) =>{
            return array.slice((page_number - 1) * page_size, page_number * page_size);
          }
          setwaitersList(paginate(response.data.data.reverse(), pageLength, currentPage))
        }
      }else{
  ...
  const _deleteWaiter = ()=>{
    ...
    .then(function (response:any) {
      ...
      setidForDelete(null)
      if(response.data.status){
        setOpenSnackbar({ ...openSnackbar, open: true,message:'Delete waiter successful !',type:'success' });
        _listOfWaiters()
      }else{
        setOpenSnackbar({ ...openSnackbar, open: true,message:'Something went wrong !',type:'error' });
      }
  ...
  return (
    ...
    <Grid style={{display:'flex',flexDirection:'row',alignItems:'center'}}>
      <Typography variant="h1" style={{fontSize:20,color:_.colors.colorBlack,fontWeight:'bold'}} component="div">Waiter List</Typography>
      <Typography variant="h1" style={{fontSize:20,color:_.colors.colorBlack,fontWeight:'bold',backgroundColor:_.colors.colorOrangeDisable,padding:10,borderRadius:10,marginLeft:10}} component="div">{totalNumberOfWaiters}</Typography>
    </Grid>
    <Grid lg={12} sm={12} md={12} xs={12} style={{padding:10,marginBottom:'10%'}}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'flex-end',marginBottom:2 }}>
            ...
            <Box sx={{display:'flex',width:'70%',alignItems:'center',justifyContent:'flex-end'}}>
                <Button onClick={()=>navigation('/admin/waiter/add-waiter')} sx={{backgroundColor:'#FF9A03',":hover":{backgroundColor:'#E18701'},marginBottom:0.5,marginLeft:2}} size='small' variant="contained">Add New waiter</Button>
            </Box>
        </Box>
        {totalNumberOfWaiters === 0?
          <Box sx={{width:'900px'}}>
            <Empty/>
          </Box>
        :<Box sx={{overflowX: 'auto', width: '100%'}}><Table sx={{minWidth: 650}} aria-label="simple table">
          ...
          <TableBody>
            {waitersList.map((row:any,index:any) => (
              <TableRow
                ...
                <TableCell align="center" >{<ActiveInactive item={row} setloading={setloading} setOpenSnackbar={setOpenSnackbar} openSnackbar={openSnackbar} _listOfWaiters={_listOfWaiters}/>}</TableCell>
                <TableCell align="center">{<ActionView item={row} setdialog={setdialog} setidForDelete={setidForDelete}/>}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table></Box>}
        {totalNumberOfWaiters !== 0 && <Pagination page={currentPage} onChange={(event: React.ChangeEvent<unknown>, page: number)=>{setcurrentPage(page)}} count={Math.ceil(totalNumberOfWaiters/pageLength)} sx={{marginTop:3}}/>}
        </TableContainer>
      </Grid>
    </Grid>
