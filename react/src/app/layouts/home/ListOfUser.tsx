...
const theme = createTheme({
  ...
  const ActiveInactive = (props:any)=>{
    const {item,setloading,setOpenSnackbar,openSnackbar,_listOfUsers} = props;
    ...
    const _updateUserStatus =()=>{
      ...
      return(
        <Box style={{display:'flex',alignItems:'center',justifyContent:'center'}}>
            <IOSSwitch sx={{ m: 1 }} defaultChecked={item.active_inactive} onChange={_updateUserStatus} />
        </Box>
      )
  }
  ...
  const ActionView = (props:any)=>{
    ...
    return(
      <Box style={{display:'flex',alignItems:'center',justifyContent:'center'}}>
        <Button onClick={()=>navigation('/admin/user/add-user',{ state: {TAG:'EDIT',data:item} })} variant="outlined" color={'primary'} size="small" startIcon={<GoPencil color={_.colors.colorSubTitle} style={{marginRight:-8,marginTop:2,marginBottom:2}} size={16}/>}/>
        <Button onClick={()=>{
          setdialog(true)
          setidForDelete(item)
  ...
  const RestaurantLogo = (props:any)=>{
    ...
    return(
        <Box sx={{}}>
            <img src={item.profile?item.profile.includes("https")?item.profile:FILE_URL+FILE_PATH.PROFILE_IMAGE+item.profile:_.images.user} 
...
export default function ListOfUser() {
  ...
  const _listOfUsers = ()=>{
    ...
    .then(function (response:any) {
      ...
      if(response.data.status){
        ...
        setusersList(paginate(response.data.data.reverse(), pageLength, currentPage))
      }else{
  ...
  const _deleteUser = ()=>{
    ...
    .then(function (response:any) {
      ...
      setidForDelete(null)
      if(response.data.status){
        setOpenSnackbar({ ...openSnackbar, open: true,message:'Delete user successful !',type:'success' });
        _listOfUsers()
      }else{
        setOpenSnackbar({ ...openSnackbar, open: true,message:'Something went wrong !',type:'error' });
      }
  ...
  return (
    ...
    <Grid style={{display:'flex',flexDirection:'row',alignItems:'center'}}>
        <Typography variant="h1" style={{fontSize:20,color:_.colors.colorBlack,fontWeight:'bold'}} component="div">User List</Typography>
        <Typography variant="h1" style={{fontSize:20,color:_.colors.colorBlack,fontWeight:'bold',backgroundColor:_.colors.colorOrangeDisable,padding:10,borderRadius:10,marginLeft:10}} component="div">{totalNumberOfUsers}</Typography>
    </Grid>
    <Grid lg={12} sm={12} md={12} xs={12} style={{padding:10,marginBottom:'10%'}}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'flex-end',marginBottom:2 }}>
            ...
            <Box sx={{display:'flex',width:'70%',alignItems:'center',justifyContent:'flex-end'}}>
                <Button onClick={()=>navigation('/admin/user/add-user')} sx={{backgroundColor:'#FF9A03',":hover":{backgroundColor:'#E18701'},marginBottom:0.5,marginLeft:2}} size='small' variant="contained">Add New User</Button>
            </Box>
        </Box>
          ...
          <TableBody>
            {usersList.map((item:any,index:any) => (
              <TableRow
                ...
                <TableCell align="center"><Typography variant="h2" sx={{marginLeft:1,maxWidth:'200px',textAlign:'start',fontSize:14,color:_.colors.colorTitle,display: '-webkit-box',overflow: 'hidden',WebkitBoxOrient: 'vertical',WebkitLineClamp:2}} component="div">{item.name}</Typography></TableCell>
                <TableCell align="center"><Typography variant="h2" sx={{marginLeft:1,maxWidth:'200px',textAlign:'start',fontSize:14,color:_.colors.colorTitle,display: '-webkit-box',overflow: 'hidden',WebkitBoxOrient: 'vertical',WebkitLineClamp:2}} component="div">{item.phone?item.phone:'N/A'}</Typography></TableCell>
                <TableCell align='center'>{item.email?item.email:'N/A'}</TableCell>
                <TableCell align="center" >{<ActiveInactive item={item}setloading={setloading} setOpenSnackbar={setOpenSnackbar} openSnackbar={openSnackbar} _listOfUsers={_listOfUsers}/>}</TableCell>
                <TableCell align="center">{<ActionView item={item} setdialog={setdialog} setidForDelete={setidForDelete}/>}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table></Box>}
        {totalNumberOfUsers !== 0 && <Pagination page={currentPage} onChange={(event: React.ChangeEvent<unknown>, page: number)=>{setcurrentPage(page)}} count={Math.ceil(totalNumberOfUsers/pageLength)} sx={{marginTop:3}} />}
        </TableContainer>
      </Grid>
