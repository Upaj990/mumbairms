...
const theme = createTheme({
  ...
  const ActiveInactive = (props:any)=>{
    const {item,setloading,setOpenSnackbar,openSnackbar,_listOfInventory} = props;

    const _updateInventoryStatus =()=>{
        setloading(true)
      ...
      return(
        <Box style={{display:'flex',alignItems:'center',justifyContent:'center'}}>
          <IOSSwitch size='small' sx={{ m: 1 }} defaultChecked={item.active_inactive} onChange={_updateInventoryStatus}/>
        </Box>
      )
    }   
    const ActionView = (props:any)=>{
        ...
        return(
            <Box style={{display:'flex',alignItems:'center',justifyContent:'center'}}>
                <Button onClick={()=>navigation('/admin/inventory/add-inventory',{ state: {TAG:'EDIT',data:item} })} variant="outlined" color={'primary'} size="small" startIcon={<GoPencil color={_.colors.colorSubTitle} style={{marginRight:-8,marginTop:2,marginBottom:2}} size={16}/>}/>
                <Button onClick={()=>{
                  setdialog(true)
                  setidForDelete(item)
      ...
      export default function ListOfInventory() {
        ...
        const [totalNumberOfInventory, settotalNumberOfInventory] = useState(0)
        ...
        useEffect(() => {
          ...
          const _listOfInventory = ()=>{
            ...
            var config = {
              method: 'post',
              url: BASE_URL+PATH.LIST_OF_INVENTORY,
              headers: { 
                'Content-Type': 'application/x-www-form-urlencoded'
              },
              data : data
      ...
      if(response.data.status){
        if(userDetails?.user.admin_type === 'MANAGER' || userDetails?.user.admin_type === 'WAITER' || userDetails?.user.admin_type === 'CHEF'){
          settotalNumberOfInventory(response.data.data.filter((a:any)=>a?.restaurant?._id === userDetails?.user.restaurant._id).length)
          const paginate = (array:any,page_size:any,page_number:any) =>{
            return array.slice((page_number - 1) * page_size, page_number * page_size);
          }
          setinventoryList(paginate(response.data.data.filter((a:any)=>a?.restaurant?._id === userDetails?.user.restaurant._id).reverse(), pageLength, currentPage))
        }else{
          settotalNumberOfInventory(response.data.data.length)
          const paginate = (array:any,page_size:any,page_number:any) =>{
            return array.slice((page_number - 1) * page_size, page_number * page_size);
          }
          setinventoryList(paginate(response.data.data.reverse(), pageLength, currentPage))
        }
      }
    })
            .catch(function (error:any) {
          ...
          const _deleteInventory = ()=>{
            ...
            var config = {
              method: 'post',
              url: BASE_URL+PATH.DELETE_INVENTORY,
              headers: { 
                'Content-Type': 'application/x-www-form-urlencoded'
              },
              data : data
            ...
            .then(function (response:any) {
              ...
              setidForDelete(null)
              if(response.data.status){
                setOpenSnackbar({ ...openSnackbar, open: true,message:'Delete Inventory successful !',type:'success' });
                _listOfInventory()
              }else{
                setOpenSnackbar({ ...openSnackbar, open: true,message:'Something went wrong !',type:'error' });
              }
          ...
          return (
            ...
            <Grid style={{display:'flex',flexDirection:'row',alignItems:'center'}}>
                <Typography variant="h1" style={{fontSize:20,color:_.colors.colorBlack,fontWeight:'bold'}} component="div">Inventory List</Typography>
                <Typography variant="h1" style={{fontSize:20,color:_.colors.colorBlack,fontWeight:'bold',backgroundColor:_.colors.colorOrangeDisable,padding:10,borderRadius:10,marginLeft:10}} component="div">{totalNumberOfInventory}</Typography>
            </Grid>
            <Grid lg={12} sm={12} md={12} xs={12} style={{padding:10,marginBottom:'10%'}}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'flex-end',marginBottom:2 }}>
                    ...
                    <Box sx={{display:'flex',width:'70%',alignItems:'center',justifyContent:'flex-end'}}>
                        <Button onClick={()=>navigation('/admin/inventory/add-inventory')} sx={{backgroundColor:'#FF9A03',":hover":{backgroundColor:'#E18701'},marginBottom:0.5,marginLeft:2}} size='small' variant="contained">Add Inventory</Button>
                    </Box>
                </Box>
                {totalNumberOfInventory === 0?
                  <Box sx={{width:'900px'}}>
                    <Empty/>
                  </Box>
                :<Box sx={{overflowX: 'auto', width: '100%'}}><Table sx={{minWidth:'900px'}} aria-label="simple table">
                  <TableHead>
                    <TableRow style={{backgroundColor:'#F0F0F0'}}>
                      ...
                      <TableCell align="center" style={{color:_.colors.colorBlack}}>TYPE</TableCell>
                       {/*<TableCell align="center" style={{color:_.colors.colorBlack}}>RESTAURANT</TableCell> */}
                      <TableCell align="center" style={{color:_.colors.colorBlack}}>TOTAL-STOCK(KG)</TableCell>
                      <TableCell align="center" style={{color:_.colors.colorBlack}}>TOTAL-USED STOCK(KG)</TableCell>
                      <TableCell align="center" style={{color:_.colors.colorRed}}>REMAINING(KG)</TableCell>
                  ...
                  <TableBody>
                    {inventoryList.map((item:any,index:any) => (
                      <TableRow
                        ...
                        <TableCell align="center"><Typography variant="h2" sx={{textAlign:'center',fontSize:14,color:_.colors.colorTitle,display: '-webkit-box',overflow: 'hidden',WebkitBoxOrient: 'vertical',WebkitLineClamp:2}} component="div">{item.item_name}</Typography></TableCell>
                        <TableCell align="center"><Typography variant="h2" sx={{textAlign:'center',fontSize:14,color:_.colors.colorTitle,display: '-webkit-box',overflow: 'hidden',WebkitBoxOrient: 'vertical',WebkitLineClamp:2}} component="div">{item?.stockType?.name}</Typography></TableCell>
                    {/*<TableCell align="center"><Typography variant="h2" sx={{marginLeft:1,maxWidth:'200px',textAlign:'start',fontSize:14,color:_.colors.colorTitle,display: '-webkit-box',overflow: 'hidden',WebkitBoxOrient: 'vertical',WebkitLineClamp:2}} component="div">{item?.restaurant?.restaurant_name}</Typography></TableCell> */}
                        <TableCell align="center"><Typography variant="h2" sx={{textAlign:'center',fontSize:14,color:_.colors.colorTitle,display: '-webkit-box',overflow: 'hidden',WebkitBoxOrient: 'vertical',WebkitLineClamp:2}} component="div">{(item.total_amount).toFixed(2)}</Typography></TableCell>
                        <TableCell align="center"><Typography variant="h2" sx={{textAlign:'center',fontSize:14,color:_.colors.colorTitle,display: '-webkit-box',overflow: 'hidden',WebkitBoxOrient: 'vertical',WebkitLineClamp:2}} component="div">{(item.current_amount).toFixed(2)}</Typography></TableCell>
                        <TableCell align="center"><Typography variant="h2" sx={{textAlign:'center',fontSize:14,color:_.colors.colorRed,display: '-webkit-box',overflow: 'hidden',WebkitBoxOrient: 'vertical',WebkitLineClamp:2}} component="div">{(item.total_amount - item.current_amount).toFixed(2)}</Typography></TableCell>
                        <TableCell align="center" >{<ActiveInactive item={item} setloading={setloading} setOpenSnackbar={setOpenSnackbar} openSnackbar={openSnackbar} _listOfInventory={_listOfInventory}/>}</TableCell>
                        <TableCell align="center">{<ActionView item={item} setdialog={setdialog} setidForDelete={setidForDelete}/>}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table></Box>}
                {totalNumberOfInventory !== 0 && <Pagination page={currentPage} onChange={(event: React.ChangeEvent<unknown>, page: number)=>{setcurrentPage(page)}} count={Math.ceil(totalNumberOfInventory/pageLength)} sx={{marginTop:3}} />}
                </TableContainer>
              </Grid>
            </Grid>
